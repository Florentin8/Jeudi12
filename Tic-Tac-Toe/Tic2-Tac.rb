...
def check_for_winner
  win_combos.each do |combos|
    if combos[0] == "O" && combos[1] == "O" && combos[2] == "O"
      puts "Player2 est le winnwur! Good luck next time."
      exit
    elsif combos[0] == "X" && combos[1] == "X" && combos[2] == "X"
      puts "Player1 est le winneur! Good job!"
      exit
    #else
...
def start_game
  ...
  puts "Pour placer le x sur la grille, entrer la location(ex--a1, b3)."
  puts "Pour quiter, entrer q ou quit"
end
